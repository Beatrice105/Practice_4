...
alpha_determine<- function(theta,delta,grad, c1=0.1,c2=0.9,f,...){
  ...
  a <- 0 ## set a small value to reduce alpha so as to satisfy Wolfe 1
  b <- 1000000 ## set a large value to increase alpha so as to satisfy Wolfe 2
  alpha <- 1 ## initialize alpha
  j <- 0 ## number of steps of updating alpha
  while(TRUE){ ## loop over both wolfe condition 1 and wolfe condition 2
    if (wolfcon1(theta,delta,grad,alpha, c1 , f,...) > 0){
      ...
      j <- j + 1 ## increase number of steps of updating alpha
      b <- alpha ## set b as alpha's upper limit when considering wolfe condition 1
      alpha <- (a + alpha) / 2 ## decrease alpha
      next ## continue if wolfe condition 1 isn't satisfied
    }
    if (wolfcon2(theta,delta,grad,alpha, c2, f,...) < 0){
      ...
      a <- alpha ## set a as the lower limit when considering wolfe condition 2
      alpha <- min(c(2*alpha,(alpha+b)/2)) ## increase alpha
      next ## continue if wolfe condition 2 isn't satisfied
    }
    break ## break if both wolfe condition 1 and wolfe condition 2 are satisfied
  }
  return (alpha) ## return alpha satisfying both conditions
}